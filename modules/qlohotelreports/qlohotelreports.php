...
class QloHotelReports extends Module
{
    ...
    private static $reports = array(
        'reservation'      => array('label' => 'Room Reservations',    'tpl' => 'group-bookings.tpl'),
        ...
        'revenue'          => array('label' => 'Daily Revenue',          'tpl' => 'group-revenue.tpl'),
        ...
        'outstanding'      => array('label' => 'Order Due Amount',       'tpl' => 'group-revenue.tpl'),
        'occupancy'        => array('label' => 'Day-wise Occupancy',    'tpl' => 'group-occupancy.tpl'),
        ...
        'services'         => array('label' => 'Booked Services',       'tpl' => 'group-guests.tpl'),
    ...
    public function hookAdminStatsModules($params)
    {
        ...
        if (Tools::getValue('export')) {
            $bookingReports  = array('reservation', 'arrivals', 'departures', 'in-house', 'cancellation');
        ...
        if (in_array($report, array('revenue', 'refund', 'payment', 'tax', 'outstanding'))) {
            ...
            if ($report === 'revenue') {
                ...
                while ($curDate <= $dateTo) {
                    ...
                    $netRev   = $roomRev + $svcRev;
                    $totalCol = $netRev + $taxAmt;
                    $adrDay   = (int) $roomData['room_nights'] > 0 ? round($roomRev / (int) $roomData['room_nights'], 2) : 0.0;

                    $dailyRows[] = array(
                        'date'             => $curDate,
                        'rooms_sold'       => $roomsBooked,
                        'bookings'         => $bkgs,
                    ...
                    $curDate = date('Y-m-d', strtotime('+1 day', $timestamp));
                }
                ...
                $outstanding      = Order::getOutstandingBalance($baseParams);
        ...
        if (in_array($report, array('occupancy', 'availability', 'room-status', 'room-perf'))) {
            ...
            if ($report === 'occupancy') {
                ...
                $this->context->smarty->assign(array(
                    ...
                    'rooms_page_base_url'  => $filterBaseUrl . '&rooms_page=',
        ...
        if ($report === 'services' || $report === 'guest-directory') {
            ...
            if ($report === 'services') {
                ...
                $guests = Order::getGuestDirectory($baseParams);
                if ($guestType === 'new') {
        ...
        if ($report === 'source' || $report === 'payment-method') {
            ...
            if ($report === 'source') {
                ...
                $sourceTotals = array(
                    'bookings' => 0, 'rooms_booked' => 0, 'room_nights' => 0,
                    'revenue_excl' => 0.0, 'revenue_incl' => 0.0,
                    'discount_amount' => 0.0, 'refund_amount' => 0.0,
                    'tax_amount' => 0.0, 'net_revenue' => 0.0, 'cancellations' => 0,
                );
                foreach ($sourceRows as $row) {
                    $sourceTotals['bookings']        += (int) $row['bookings'];
                    $sourceTotals['rooms_booked']     += (int) $row['rooms_booked'];
                    ...
                    $sourceTotals['net_revenue']      += (float) $row['net_revenue'];
            ...
            if ($report === 'daily-summary') {
                ...
                while ($current <= $dateTo) {
                    ...
                    $dailyRows[] = array(
                        ...
                        'revpar'        => $totalRooms ? round((isset($revenueByDay[$timestamp]) ? (float) $revenueByDay[$timestamp] : 0.0) / $totalRooms, 2) : 0.0,
                ...
                foreach ($hotels as $hotel) {
                    ...
                    $bookings    = HotelBookingDetail::getTotalBookings(array_merge($hotelParams, array('count_rows' => true)));
                    ...
                    $hotelRows[] = array(
                        ...
                        'occupancy'           => AdminStatsController::getAverageOccupancyRate($dateFrom, $dateTo, $hotelId),
                        'room_revenue'        => $roomRevenue,
                        'extra_service_rev'   => $extraRev,
                        'gross_revenue'       => $roomRevenue + $extraRev,
                        'adr'                 => AdminStatsController::getAverageDailyRate($dateFrom, $dateTo, $hotelId),
                        'revpar'              => AdminStatsController::getRevenuePerAvailableRoom($dateFrom, $dateTo, $hotelId),
                        'bookings'            => $bookings,
    ...
    private function exportBookingsCsv($dateFrom, $dateTo, $idHotel, $report)
    {
        ...
        if ($report === 'arrivals' || $report === 'departures') {
            ...
            fputcsv($out, array(
                $this->l('Room Type'), $this->l('Room No.'), $this->l('Check-in'), $this->l('Check-out'),
                $this->l('Order ID'), $this->l('Guest Name'), $this->l('Hotel'),
                $this->l('Nights'), $this->l('Adults'), $this->l('Children'),
                $this->l('Total (incl. Tax)'), $this->l('Currency'),
            ));
            foreach ($rows as $row) {
                fputcsv($out, array(
                    $row['room_type_name'], $row['room_num'], $row['actual_checkin'], $row['actual_checkout'],
                    $row['id_order'], $row['customer_name'], $row['hotel_name'],
                    $row['los'], $row['adults'], $row['children'],
                    $row['total_price_tax_incl'], $row['currency_iso'],
                ));
            }
            fclose($out);
            exit;
    ...
    private function exportRevenueCsv($dateFrom, $dateTo, $idHotel, $idProduct, $report)
    {
        ...
        if ($report === 'payment') {
            ...
            fputs($out, "\xEF\xBB\xBF");
            fputcsv($out, array(
                $this->l('Payment Date'), $this->l('Payment ID'), $this->l('Order ID'),
                $this->l('Booking Ref.'), $this->l('Guest Name'),
                $this->l('Payment Method'), $this->l('Payment Type'),
                $this->l('Amount'), $this->l('Payment Status'), $this->l('Transaction Reference'),
                $this->l('Currency'),
            ));
            foreach ($rows as $row) {
                ...
                fputcsv($out, array(
                    isset($row['date_add'])        ? $row['date_add']        : '',
                    isset($row['id_order_payment']) ? (int) $row['id_order_payment'] : '',
                    $row['id_order'],
                    $row['reference'],
                    $row['customer_name'],
                    $row['payment_method'],
            ...
            fputs($out, "\xEF\xBB\xBF");
            fputcsv($out, array(
                $this->l('Refund Date'), $this->l('Refund ID'), $this->l('Order ID'),
                $this->l('Guest Name'), $this->l('Original Booking Amount'), $this->l('Refund Amount'),
                $this->l('Refund Method'), $this->l('Refund Status'), $this->l('Refund Remark'),
                $this->l('Currency'),
            ));
            foreach ($rows as $row) {
                fputcsv($out, array(
                    isset($row['cancellation_date']) ? $row['cancellation_date'] : '',
                    isset($row['id_order_return']) ? (int) $row['id_order_return'] : '',
                    $row['id_order'],
                    $row['customer_name'],
                    number_format((float) $row['total_price_tax_incl'], _PS_PRICE_DISPLAY_PRECISION_, '.', ''),
                    number_format((float) $row['refunded_amount'], _PS_PRICE_DISPLAY_PRECISION_, '.', ''),
                    isset($row['refund_method']) ? $row['refund_method'] : '',
                    isset($row['refund_status']) ? $row['refund_status'] : '',
                    isset($row['cancellation_reason']) ? $row['cancellation_reason'] : '',
                    $currencyIso,
                ));
            ...
            fputs($out, "\xEF\xBB\xBF");
            fputcsv($out, array(
                $this->l('Order ID'), $this->l('Guest Name'),
                $this->l('Guest Email'), $this->l('Guest Phone'),
                $this->l('Total Charges'), $this->l('Total Paid'), $this->l('Outstanding Balance'),
                $this->l('Last Payment Date'), $this->l('Currency'),
            ));
            foreach ($rows as $row) {
                fputcsv($out, array(
                    $row['id_order'],
            ...
            fputs($out, "\xEF\xBB\xBF");
            fputcsv($out, array(
                $this->l('Date'), $this->l('Rooms Booked'), $this->l('Total Bookings'),
                $this->l('Room Revenue (excl. Tax)'), $this->l('Extra Services Revenue'),
                $this->l('Tax Amount'), $this->l('Refund Amount'),
                $this->l('Total Collection'), $this->l('Net Revenue'),
                $this->l('ADR'), $this->l('Currency'),
            ));
            $csvDate = $dateFrom;
            while ($csvDate <= $dateTo) {
                ...
                $roomsBooked = (int) $roomData['rooms_booked'];
                $roomNights  = (int) $roomData['room_nights'];
                $adr         = $roomNights > 0 ? round($roomRevExcl / $roomNights, 2) : 0.0;
                fputcsv($out, array(
                    "\t" . $csvDate,
                    $roomsBooked,
                ...
                $csvDate = date('Y-m-d', strtotime('+1 day', $timestamp));
            }
        }
    ...
    private function exportOccupancyCsv($dateFrom, $dateTo, $idHotel, $idLang, $report)
    {
        ...
        if ($report === 'availability') {
            ...
            fputcsv($out, array(
                ...
                $this->l('Cancellation Count'), $this->l('Avg. LOS'),
                $this->l('Currency'),
            ));
            foreach ($rows as $row) {
                fputcsv($out, array(
    ...
    private function exportGuestsCsv($dateFrom, $dateTo, $idHotel, $report)
    {
        ...
        if ($report === 'services') {
            ...
            $guests = Order::getGuestDirectory(
    ...
    private function exportChannelsCsv($dateFrom, $dateTo, $idHotel, $report)
    {
        ...
        if ($report === 'source') {
            ...
            fputs($out, "\xEF\xBB\xBF");
            fputcsv($out, array(
                $this->l('Channel'), $this->l('Total Orders'), $this->l('Rooms Booked'), $this->l('Room Nights'),
            ...
            foreach ($rows as $row) {
                ...
                fputcsv($out, array(
                    $row['channel_label'],
                    $bookings,
                    isset($row['rooms_booked']) ? (int) $row['rooms_booked'] : 0,
                    $roomNights,
    ...
    private function exportPropertyCsv($dateFrom, $dateTo, $idHotel, $idLang, $report)
    {
        ...
        if ($report === 'daily-summary') {
            ...
            fputcsv($out, array(
                ...
                $this->l('In-house Guests'), $this->l('Cancellations'),
                $this->l('Currency'),
            ));
            $current = $dateFrom;
            while ($current <= $dateTo) {
                ...
                fputcsv($out, array(
                    ...
                    number_format($totalRoomsProp ? round((isset($revenueByDay[$timestamp]) ? (float) $revenueByDay[$timestamp] : 0.0) / $totalRoomsProp, 2) : 0.0, _PS_PRICE_DISPLAY_PRECISION_, '.', ''),
            ...
            fputcsv($out, array(
                ...
                $this->l('Total Room Bookings'), $this->l('Total Cancellations'), $this->l('Cancel Rate %'),
                $this->l('Avg LOS'), $this->l('Due Amount'),
                $this->l('Currency'),
            ));
            foreach ($hotels as $hotel) {
                ...
                fputcsv($out, array(
                    ...
                    number_format((float) AdminStatsController::getAverageDailyRate($dateFrom, $dateTo, (int) $hotel['id']), _PS_PRICE_DISPLAY_PRECISION_, '.', ''),
                    number_format((float) AdminStatsController::getRevenuePerAvailableRoom($dateFrom, $dateTo, (int) $hotel['id']), _PS_PRICE_DISPLAY_PRECISION_, '.', ''),
                    $bookings,
                    $cancels,
                    number_format($cancelRate, 1, '.', '').'%',
            ...
            fputcsv($out, array(
                ...
                $this->l('Start Date'), $this->l('End Date'),
